...
export default async function InspectorProfilePage({ params }: { params: { slug: string } }) {
  ...
  const badges = (company.inspector_badges || [])
    .filter((b: Badge) => b.badge_definitions)
    .sort((a: Badge, b: Badge) => (a.badge_definitions?.sort_order || 0) - (b.badge_definitions?.sort_order || 0))

  const isVerified = badges.some((b: Badge) => b.badge_code === 'verified_pro')
